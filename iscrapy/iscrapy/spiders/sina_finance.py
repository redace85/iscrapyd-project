...
# JS decoder for Sina Finance proprietary KLC/KLC2 encoding.
# Adapted from https://github.com/sfl666/option_tools (MIT), which in turn
# derives from the AKShare project.  The d() function handles all Sina KLC
# subtypes (OHLCV = type 1479, trading-calendar = type 139, etc.).
# Node.js reads the encoded string from stdin and writes JSON to stdout.
_NODE_SCRIPT = r"""
function d(t) {
...
process.stdin.on('end', () => {
    try {
        const rows = d(_raw.trim());
        const out = rows.map(row => {
            if (row instanceof Date) return row.toISOString().slice(0, 10);
            const obj = {};
            for (const key of Object.keys(row))
                obj[key] = row[key] instanceof Date ? row[key].toISOString().slice(0, 10) : row[key];
            return obj;
        });
...
class SinaFinanceSpider(scrapy.Spider):
    ...
    # stock/index symbol, e.g. sz399006 (ChiNext), sh000001 (SSE Composite)
    symbol = "sz399006"
    ...
    def start_requests(self):
        ...
        self.url = (
            f"https://finance.sina.com.cn/realstock/company/{self.symbol}"
            f"/hisdata/klc_kl.js"
        )
    ...
    def _parse_calendar(self, response):
        # Decode the trading calendar (type-139 / k-function).
        # The resulting list of date strings represents actual trading days
        # (holidays excluded), which can be used to validate or correct
        # dates from the OHLCV decoder when needed.
        m = re.search(r'datelist="([^"]+)"', response.text)
        if m:
            try:
                self.trading_dates = self._node_decode(m.group(1))
            except Exception:
                self.trading_dates = []
        else:
            self.trading_dates = []
    ...
    def parse(self, response):
        ...
        try:
            ...
            if not m:
                raise ValueError("encoded data not found in response")
            records = self._node_decode(m.group(1))
            for rec in records:
                ...
                yield IscrapyItem(item_id=ts, data=data, msg="", failed=False)
        except Exception as e:
            msg = f"spider: {self.name} url: {self.url} error: {e}"
            yield IscrapyItem(msg=msg, failed=True)

    @staticmethod
    def _node_decode(encoded: str) -> list:
        proc = subprocess.run(
            ["node", "-e", _NODE_SCRIPT],
            input=encoded,
            capture_output=True,
        ...
        if proc.returncode != 0:
            raise RuntimeError(proc.stderr.strip() or "node decode error")
